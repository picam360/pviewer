var create_plugin = (function() {
	...
	var keydownFun = function (ev) {
		...
		case "ArrowLeft":
			if(m_pst){
				m_pstcore.pstcore_set_param(m_pst, m_loader_name, "skip", "-30");
			}
			break;
		case "ArrowRight":
			if(m_pst){
	...
	function on_pvf_started() {
			...
			switch (e.type) {
				case "up" :
					if(m_play_button.pause){
						m_pstcore.pstcore_set_param(m_pst, m_loader_name, "pause", "0");
					}else{
						m_pstcore.pstcore_set_param(m_pst, m_loader_name, "pause", "1");
					}
					break;
			}
		});
		...
		m_play_button.update_interval = setInterval(() => {
			var paus_str = m_pstcore.pstcore_get_param(m_pst, m_loader_name, "pause");
		...
		m_slider.addEventListener("change", (e) => {
			...
			m_pstcore.pstcore_set_param(m_pst, m_loader_name, "set_pts", pts.toString());
		...
		m_slider.update_interval = setInterval(() => {
			...
			get_pts((st_pts, et_pts, pts) => {
				m_st_pts = st_pts;
				m_et_pts = et_pts;
				m_pts = pts;
				m_slider.value = (m_pts - m_st_pts) / (m_et_pts - m_st_pts) * 1000;
				...
				var ct_s = parseInt(m_pts - m_st_pts);
	...
	function get_pts(callback) {
		var st_pts = m_pstcore.pstcore_get_param(m_pst, m_loader_name, "st_pts");
		var et_pts = m_pstcore.pstcore_get_param(m_pst, m_loader_name, "et_pts");
		var pts = m_pstcore.pstcore_get_param(m_pst, m_loader_name, "pts");
		callback(parseFloat(st_pts), parseFloat(et_pts), parseFloat(pts));
	}

	function on_pst_started() {
		m_pvf_chcker = setInterval(() => {
			get_pts((st_pts, et_pts, pts) => {
				if(!isNaN(st_pts) && !isNaN(et_pts) && !isNaN(pts)){
					m_st_pts = st_pts;
					m_et_pts = et_pts;
					m_pts = pts;
