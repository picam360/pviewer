...
function MeetingClient(host) {
	...
	var m_pst_in;
	var m_pst_in2;
	var m_enqueue_pendings_out = [];
	var m_enqueue_pendings_in = [];
	...
	var delay = [];
	{
		...
		pstcore.pstcore_build_pstreamer(def, (pst) => {
			m_pst_out = pst;
			pstcore.pstcore_set_dequeue_callback(m_pst_out, (data)=>{
				try{
					if(data == null){//eob
					...
					//debug
					//pstcore.pstcore_enqueue(m_pst_in, data);
					// delay.push(data);
					// if(delay.length > 900){
					// 	var _data = delay.pop();
					// 	pstcore.pstcore_enqueue(m_pst_in2, _data);
					// }
				}catch(err){
					console.log(err);
				}
			});
			pstcore.pstcore_start_pstreamer(m_pst_out);
		});
	}
	{
		...
		pstcore.pstcore_build_pstreamer(def, (pst) => {
			m_pst_in = pst;
			pstcore.pstcore_start_pstreamer(m_pst_in);
		});
	}
	// {
	// 	var def = "opus_decoder ! oal_player realtime=1 buffer=0.2";
	// 	pstcore.pstcore_build_pstreamer(def, (pst) => {
	// 		m_pst_in2 = pst;
	// 		pstcore.pstcore_start_pstreamer(m_pst_in2);
	// 	});
	// }
			...
			if (packet.GetPayloadType() == PT_MT_ENQUEUE) { // enqueue
				...
				if(chunk[0] == eob.charCodeAt(0) &&
				   ...
				   chunk[chunk.length - 1] == eob.charCodeAt(5)){
					...
					var buff = null;
					if(m_enqueue_pendings_in.length == 1){
						buff = m_enqueue_pendings_in[0];
					}else if(m_enqueue_pendings_in.length > 1){
						var len = 0;
						for (var _chunk of m_enqueue_pendings_in) {
							len += _chunk.length;
						}
...
//							console.log("enqueue:", buff.length, size, buff[4]);
						...
						pstcore.pstcore_enqueue(m_pst_in, buff);
						pstcore.pstcore_enqueue(m_pst_in, null);//eob
						m_enqueue_pendings_in = [];
					}
				}else{
					m_enqueue_pendings_in.push(chunk);
				}
		...
		close : () => {
			pstcore.pstcore_stop_pstreamer(m_pst_out);
			pstcore.pstcore_destroy_pstreamer(m_pst_out);
			pstcore.pstcore_stop_pstreamer(m_pst_in);
			pstcore.pstcore_destroy_pstreamer(m_pst_in);
		},
	};
	return self;
}
